...
class StorefrontCog(commands.Cog):
    ...
    async def _handle_open_ticket(
        ...
        try:
            ...
            logger.debug("Preparing payment methods | User: %s | Balance: %s cents | Final price: %s cents",
                         interaction.user.id, user_balance_cents, final_price_cents)
            payment_methods = []
            ...
            owner_embed.add_field(
                name="⚠️ Ticket Information",
                value=(
                    "• This ticket will automatically close after **48 hours** of inactivity\n"
                    "• You'll receive a warning before automatic closure\n"
                    "• Send any message to keep the ticket active"
                ),
                inline=False,
            )
            ...
            logger.debug("Sending ticket messages to channel | Channel: %s | User: %s", channel.name, interaction.user.id)
            await channel.send(
            ...
            logger.debug("Sending DM to user | User: %s", interaction.user.id)
            try:
