...
def test_load_example_config() -> None:
    cfg = load_app_config(Path("configs/example.yaml"), settings=RuntimeSettings())
...
def test_cadence_env_override_is_validated() -> None:
    ...
    cfg = load_app_config(Path("configs/example.yaml"), settings=settings)
    assert cfg.scheduler.cadence_hours == 24

    with pytest.raises(ValueError):
        settings = RuntimeSettings(cadence_hours=1)
        load_app_config(Path("configs/example.yaml"), settings=settings)
